...
parser.add_argument("-k", "--kind",type=int,default="biom",help="Kind of intput file (default=biom) (options = biom,tsv)", required=False)
...
if type_file=="biom":
    ...
    ############################# import otu information from .biom ###################################################
...
if type_file=="tsv":
    ...
    ############################# import otu information from .tsv ###################################################


    print '\nImporting .tsv table for analysis \n'


    otu=pd.read_table('%s'%in_biom, index_col=0)
    taxonomy=otu.index.values.tolist()
    new_tax_level=[]
    for t in taxonomy:
        new_tax_level.append(.join(t.split(',')[txlvl:]))

    #add taxa names to specified level
    otu = result.set_index(new_tax_level)
...
for bestclassifier in mybest_classer_list[:classnum_to_analy]:
	...
	for pc in PC_list:
		...
		for bact in bact_list[:bactnum_for_pca]:
			...
			def2=def3.T[-1:]
			def2=def2.T
			def2['%s_labels'%bestclassifier] = map_names[bestclassifier]
			...
			def2tmp=def3tmp.T[-1:]
			def2tmp=def2tmp.T
			def2tmp['%s_labels'%bestclassifier] = map_names[bestclassifier]
