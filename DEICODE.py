...
parser.add_argument("-c", "--classnum",type=int,default=2,help="Number of highest scoring classifiers to use in analysis (default=2) (options = any integer greater than 1 and less than the umber of columns in the mapping file)", required=False)
parser.add_argument("-t", "--taxause",type=str,default='family',help="What level of taxonomy to extract from PCA axis (deafult=family) (options = phylum, class, order, family, genus, species)", required=False)
...
def convert_biom_to_pandas(table):
	...
	taxonomy = pd.DataFrame(mapping, index=['kingdom', 'phylum', 'class', 'order', 'family', 'genus', 'species']).T
...
for bestclassifier in mybest_classer_list[:classnum_to_analy]:
	...
	#cont 
		...
		Y=map_names[bestclassifier].tolist()
		X =low_rank_matrix.T
		X_reduced = PCA(n_components=3).fit_transform(X)
		bmap7 = brewer2mpl.get_map('Set3','qualitative',12,reverse=True)
		...
		ax.legend(scatter_proxy, k, numpoints = 1,bbox_to_anchor=(0., .89, 1., .102), loc=3, ncol=len(k),fontsize = 'large', labelspacing=.2, borderaxespad=0.)
	...
	for pc in PC_list:
		...
		ax1.legend(bbox_to_anchor=(0., .89, 1., .102), loc=3, ncol=int(int(len(set(map_names[bestclassifier].tolist())))/4),fontsize = 'large', labelspacing=.2, borderaxespad=0.)
