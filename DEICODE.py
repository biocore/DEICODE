...
parser.add_argument("-d", "--decompit",type=int,default=100,help="How many iterations to complete in decomposition (deafault=100) (options = any integer)", required=False)
...
if lr_method=="WPCA" or lr_method=="EMPCA": # WPCA or EMPCA specified
    ...
    if lr_method=="EMPCA":
        print("Running EMPCA")
        low_rank_matrix = EMPCA(n_components=3).fit_reconstruct(otum,weight)
    else:
        print("Running WPCA")
        low_rank_matrix = EMPCA(n_components=3).fit_reconstruct(otum,weight)
    ...
    if lr_method=='KNN':
        ...
        low_rank_matrix=IterativeSVD(rank=(min(otum.shape)-1),convergence_threshold=0.00001,max_iters=iteration_used,gradual_rank_increase=False,svd_algorithm="arpack",init_fill_method="zero",min_value=0,max_value=(np.amax(otum)),verbose=True).complete(otum)
    elif lr_method=='MICE':
        print("Running MICE")
        ...
        low_rank_matrix=SoftImpute(shrinkage_value=None,convergence_threshold=0.00001,max_iters=iteration_used,max_rank=min(otum.shape),n_power_iterations=1,init_fill_method="zero",min_value=0,max_value=(np.amax(otum)),normalizer=None,verbose=True).complete(otum)
...
for metatmp in classifiers_meta[mapstart_num:]: # run each classifier
    ...
    if len(set(encoded_mapping[metatmp][0]))>2 and 4>len(set(encoded_mapping[metatmp][0])): # if not continuous but more than two use linear support vector classifier
...
print('\n Saving Classifier Scores and Visualizations for each classifier \n')
...
# run classnum_to_analy number of the best classifiers
...
############################# Plot OTUS affecting each classifier for best axis of PCA ########################################################
    ...
    for pc in PC_list:
        ...
        if len(set(encoded_mapping[metatmp][1])) > 12:
            cont=True # true, false if not continous data
        else:
            cont=False
        # Extract information from imputed PCA axis
        out_niche_linkeddf,observed_table_sfi,index_mean,index_std = PCA_niche.niche_visual(otu,low_rank_matrix,tax_index,bactnum_for_pca,bestclassifier,pc,mappingdf)
        # Visualize the data
