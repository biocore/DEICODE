...
#ranks
ranks={}
ranks['AG'] = 3 # three clusters here
...
for dataset_ in [dir_ for dir_ in os.listdir('data/') if dir_!='.DS_Store']:
    ...
    for sub_set in [dir_ for dir_ in os.listdir(subpath_) if dir_!='.DS_Store']:
        ...
        # bray-curtis
        jaccard_dist = os.path.join(dir_path,subpath_,sub_set,'Jaccard_Distance.qza')
        subprocess.call("qiime diversity beta --i-table %s --p-metric 'jaccard' --o-distance-matrix %s"%(table_path_qiime,jaccard_dist), shell=True)
        ...
        # generalized weighted alpha=0.5
        whalf_dist = os.path.join(dir_path,subpath_,sub_set,'GUniFrac_alpha_half_Distance.qza')
        subprocess.call("qiime diversity beta-phylogenetic-alt --i-table %s --i-phylogeny %s --p-metric generalized_unifrac --p-alpha 0.5 --o-distance-matrix %s"%(table_path_qiime,tree_path_qiime,whalf_dist), shell=True)
        # generalized weighted alpha=0.0
        wzero_dist = os.path.join(dir_path,subpath_,sub_set,'GUniFrac_alpha_zero_Distance.qza')
        subprocess.call("qiime diversity beta-phylogenetic-alt --i-table %s --i-phylogeny %s --p-metric generalized_unifrac --p-alpha 0.0 --o-distance-matrix %s"%(table_path_qiime,tree_path_qiime,wzero_dist), shell=True)                
        # extract txt files from each distance
        for convert_ in [bray_dist,jaccard_dist,wone_dist,whalf_dist,wzero_dist]:
