...
class rclr(_BaseTransform):
    ...
    def __init__(self):
        ...
        The rclr procedure first log transform the nonzero values before centering the data -- 
        we refer to this preprocessing procedure as the robust center log-ratio (rclr) due 
        to its ties to the clr transform commonly used in compositional data analysis.
        
        Parameters
        ----------
        
        X: numpy.ndarray - a table array of all positive count data of shape (M,N) containing zeros
    ...
    def _fit(self):
        ...
        X_log=np.log(closure(np.array(X_))) #log of all values
        log_mask = np.array(
