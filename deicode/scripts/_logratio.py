...
def logratio(in_biom: str, in_map: str,
             ...
             axis:int, n_lr: int, tax_level: str) -> None:
    """ Runs log ratios on import features from RPCA output"""

    table = load_table(in_biom)
    metadata=pd.read_table(in_map,index_col=0)
    ...
    tabledf,metadata=match(tabledf,metadata)

    try: 
        # try to inclide taxa if there 
        taxonomy=table.metadata_to_dataframe('observation') 
        logdf=log_ratios(tabledf, metadata, feature_loading, 
