...
class PCA_niche(object):
    ...
    def niche_visual(otu,sfi,tax_index,bact_to_show,niche_plot,Pc_plot,mappingdf,weight):
        ...
        pca_model=WPCA(n_components=3) #WPCA
        X2=plot_var_otu.as_matrix() #pd to matrix
        X_reduced_var = pca_model.fit_transform(X2,weight) #transform
    ...
    def plot_niche(out_niche_linkeddf,observed_table_sfi,mappingdf,encoded_mapping,niche_plot,Pc_plot,index_mean,index_std,le,cont,weight):

        #WPCA
        observed_table_sfi, mapping = match(observed_table_sfi.T, mappingdf)
        pca_model=WPCA(n_components=2)
        ...
        if Pc_plot=="PC-1":
            ...
            if cont==True:
                ...
                fig, (ax1, ax2) = plt.subplots(ncols=1, nrows=2, figsize=(30, 15),sharey=False)
                ...
                ax2.scatter(X_reduced2[:, 0], X_reduced2[:, 1], c=list(encoded_mapping[niche_plot][0]),cmap=plt.cm.cool,s=200)
                p=ax2.scatter(X_reduced2[:, 0], X_reduced2[:, 1], c=list(encoded_mapping[niche_plot][0]),cmap=plt.cm.cool,s=200)
            ...
            if cont==False:
                ...
                #bar
                out_niche_linkeddf.T[index_mean].plot(kind='bar',width=.78, yerr=out_niche_linkeddf.T[index_std].values.T, alpha = 0.92,error_kw=dict(elinewidth=1,capsize=2,barsabove=True,ecolor='k',ms=1, mew=1),rot=0,colormap="Set1",sharey=True,fontsize=30,ax=ax1)
        ...
        if Pc_plot=="PC-2":
            ...
            if cont==True:
                ...
                ax1.legend(loc=2,prop={'size':16},bbox_to_anchor=(1.0, 1.0))
                Y=encoded_mapping[niche_plot][0].tolist()
                ax2.scatter(X_reduced2[:, 1], X_reduced2[:, 0], c=list(encoded_mapping[niche_plot][0]),cmap=plt.cm.cool,s=200)
                p=ax2.scatter(X_reduced2[:, 1], X_reduced2[:, 0], c=list(encoded_mapping[niche_plot][0]),cmap=plt.cm.cool,s=200)
                fig.colorbar(p,orientation='horizontal')
                # Set common labels
                ax1.set_ylabel(('Mean Frequency in %s Niche'%niche_plot))
            ...
            if cont==False:
                ...
                if int(len(k))>=3:
                    ...
                    numrows=int(len(k))
                ax2.legend(scatter_proxy, k, numpoints = 1,loc=2,prop={'size':40},bbox_to_anchor=(1.0, 1.0),fontsize = 'large', labelspacing=.1, borderaxespad=0.)
                ax2.scatter(X_reduced2[:, 1], X_reduced2[:, 0], c=Y,cmap=plt.cm.Set1_r,s=350)
                # Set common labels
                ax1.set_ylabel(('Mean Frequency in %s Niche'%niche_plot), fontsize=20)
                ax1.set_xlabel('Niche', fontsize=20)
                ax2.set_xlabel('PC-2', fontsize=20)
                ax2.set_ylabel('PC-1', fontsize=20)
                ax1.set_title(('OTUs With Highest Weighted Varaince in %s'%niche_plot), fontsize=35)
                ax2.set_title(('PCA on Fully Dense Imputed OTU Table (colored by %s)'%niche_plot), fontsize=35)
                fig.set_tight_layout(True)
                return plt
